...
namespace DiscordCommunityPlugin.Misc
{
    ...
    class Client
    {
        ...
        public static void SubmitScore(ulong steamId, string songId, int difficultyLevel, int gameplayMode, int score, string signed)
        {
            //Build score object
            Score s = new Score
            {
            ...
            SharedCoroutineStarter.instance.StartCoroutine(PostScore(scoreData));
        }

        //Post a score to the server
        private static IEnumerator PostScore(byte[] proto)
        {
            ...
            {
                DiscordCommunityShared.Logger.Error(www.error);
            }
            else
            {
                DiscordCommunityShared.Logger.Success("Score upload complete!");
            }
        }
        ...
        private static IEnumerator GetUserData(SongListViewController slvc, string steamId)
        {
            ...
            if (www.isNetworkError || www.isHttpError)
            {
                DiscordCommunityShared.Logger.Error($"Error getting player stats: {www.error}");
            ...
            else
            {
                ...
                {
                    DiscordCommunityShared.Logger.Error($"Error parsing getplayerstats data: {e}");
        ...
        private static IEnumerator GetWeeklySongs(LevelCollectionsForGameplayModes lcfgm, SongListViewController slvc)
        {
            ...
            if (www.isNetworkError || www.isHttpError)
            {
                DiscordCommunityShared.Logger.Error($"Error getting weekly songs: {www.error}");
            ...
            else
            {
                ...
                {
                    DiscordCommunityShared.Logger.Error($"Error parsing getweeklysong data: {e}");
        ...
        private static IEnumerator DownloadWeeklySongs(string songId, SongListViewController slvc)
        {
            ...
            {
                DiscordCommunityShared.Logger.Error($"Downloading error: {www.error}");
                slvc.DownloadErrorHappened($"Downloading error: {www.error}");
            }
            else
            {
                //DiscordCommunityShared.Logger.Info("Received response from BeatSaver.com...");
                ...
                try
                {
                    ...
                    //DiscordCommunityShared.Logger.Info("Downloaded zip file!");
                }
                catch (Exception e)
                {
                    DiscordCommunityShared.Logger.Error($"Error writing zip: {e}");
                    slvc.DownloadErrorHappened($"Error writing zip: {e}");
                    yield break;
                }

                //DiscordCommunityShared.Logger.Info("Extracting...");
                ...
                {
                    DiscordCommunityShared.Logger.Error($"Unable to extract ZIP! Exception: {e}");
                ...
                {
                    DiscordCommunityShared.Logger.Warning($"Unable to delete zip! Exception: {e}");
                    slvc.DownloadErrorHappened($"Unable to delete zip! Exception: {e}");
                    yield break;
                }

                DiscordCommunityShared.Logger.Success($"Downloaded!");
            }
        }
    }
}
